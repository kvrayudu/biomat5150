...
class autoComplete extends CJuiAutoComplete
{
    ...
    $attr_id = get_class($this->model).'_'.$this->attribute;
      ...
      echo CHtml::activeHiddenField($this->model, $this->attribute,array('name'=>'hidden_'.$id ));
        }else {
            echo CHtml::textField($name,$this->value,$this->htmlOptions);       
//CHtml::hiddenField($name,$this->value,$this->htmlOptions);          
    ...
    if (!isset($this->options['select'])) {
      $this->options['select'] = 'js:function(event, ui) {
            $("#'.$id.'").val(ui.item.label);
            $("#hidden_'.$id.'").val(ui.item.id);
        }';
    }
	
	if (!isset($this->options['focusout'])) {
      $this->options['focusout'] = 'js:function(event, ui) {
