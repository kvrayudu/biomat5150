...
class GroupsController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST))
		{
			...
			foreach($names as $index => $mat_name){
				if ($mat_name != "") {
					$id_rows = Yii::app()->db->createCommand()
					->selectDistinct('id')
					->from('Material')
					->where('Name="'. $mat_name.'"')
					->queryAll();
			...
			$new_group->MaterialsInGroup = ';'.implode(';', $ids) .';';
			// $model->attributes=$_POST['Groups'];

			if($new_group->save()){
				// $this->redirect(array('view','id'=>$new_group->id));
			}
		}
	...
	public function actionList(){
		...
		foreach($groups as $key =>$group){
			$ids = explode(";", $group->MaterialsInGroup);
			$str = "";
			foreach ($ids as $key => $id) {
				$str = $str . Material::model()->findByPk(intval($id))->Name . ", ";
			}
			$names[$group->GroupName] = $str;
		}
